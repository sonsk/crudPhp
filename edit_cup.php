...
// declaration des variables en route pour mon crud 
    ...
    $taille=htmlentities($_POST['nombre']);
    $edit=$_GET['id'];

// mis à jour des informations du formulaire  récupéré par le $_GET['id']=$edit
    $req="UPDATE tblcup SET nom=:nom , tel=:tel , adresse=:adresse ,glacage=:glacage ,date_livraison=:date_livraison  , date_creation=NOW() WHERE id=:edit";
   ...
   if(isset($resultat)){
    ...
    echo "<script type= 'text/javascript'>document.location = 'manage-cupcake.php';</script>";
...
if (isset($_GET['delete'])){
    $id = $_GET['delete'];
    $sql= "DELETE FROM tblcup WHERE id=".$id;
    $query=$bdd->prepare($sql);
    $result=$query->execute();

    if($result){
        echo "<script>alert('cool');</script>";
        echo "<script type= 'text/javascript'>document.location = 'manage-cup.php';</script>";
...
<div class="container pt-3"  >
                        ...
                        <?php
                            $edit=$_GET['id']; //lorsque la barre d'adresse aura un GET id il recuperera les infos pour l'entite correspondant
                             $req="SELECT * FROM tblcup WHERE id=$edit ";
                             $query=$bdd->prepare($req);
                             $query->execute();
...
<form class="box needs-validation" action="" novalidate method="POST">
            ...
            <div class="form-group">
                <input type="number" class="form-control form-control-lg" name="nombre" placeholder="10,20,100" required value="<?php echo $result['nombre'];?>">
                <div class="invalid-feedback">Veuillez remplir correctement ce champ!</div>
            </div>
